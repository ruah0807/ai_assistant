...
def run_with_tools(ASSISTANT_ID, thread):
    ...
    run = client.beta.threads.runs.create(
        ...
        3.	코드 식별 과정:
            ...
            •	'한글'이나 'English'(column)내의 내용이 이미지 분석과 정확히 일치하는 경우, 해당 row에 있는 '6자리 코드'를 반환합니다.
            •	정확히 일치하는 코드가 없을 경우, 가장 유사한 항목과 연결된 코드를 반환합니다.
        ...
        “이미지를 분석하고, 추상적인 형태의 다음 특징들에 기반하여 Vienna 분류 코드에서 가장 유사한 코드를 식별하세요:
            ...
            •	텍스트: 이미지에 포함된 텍스트가 있는 경우, 텍스트의 언어, 스타일, 의미를 분석합니다. 텍스트가 이미지와 어떻게 통합되어 있는지, 특정 품목이나 상징을 나타내는지 확인합니다.
                - 텍스트에 특정 품목이 포함된 경우, 해당 품목과 관련된 코드를 찾습니다. 
                - 예 ) "일타르타르트"와 같은 텍스트가 이미지 내에 존재하면, "타르트"로 관련 코드를 찾습니다.
